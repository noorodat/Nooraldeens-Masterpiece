<nav>
        <div class="container position-relative">
            <div class="logo">
                <a href="/masterpiece/Nooraldeens-Masterpiece/UniVerse/Pages/Home/Home.php"><span class="logo-text">UniVerse <i
                            class="fa-solid fa-building-columns logo-icon"></i></span></a>
            </div>
            <div id="menu"><i class="fa fa-bars"></i></div>
            <ul class="links" id="navLinks">
                <li id="discountsLink"><a href="">Discounts</a></li>
                <li><a href="/masterpiece/Nooraldeens-Masterpiece/UniVerse/Pages/Home/Home.php">Home</a></li>
                <li><a href="">About</a></li>
                <li><a href="#services">Services</a></li>
                <li><a href="">Contact</a></li>
                <li id="login"><a href="/masterpiece/Nooraldeens-Masterpiece/UniVerse/Pages/Login/login.php">Login</a></li>
            </ul>
        </div>
</nav>
